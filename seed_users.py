...
def seed_users_from_csv(csv_file_path: str = "db/users_seed.csv", skip_existing: bool = True):
    """
                ...
                for row in reader:
                    try:
                        ...
                        if not email or not password:
                            print(f"⚠️  Skipping row with missing email or password")
                            error_count += 1
                            continue
...
def main():
    ...
    parser.add_argument('--csv', default='db/users_seed.csv', help='Path to CSV file')
